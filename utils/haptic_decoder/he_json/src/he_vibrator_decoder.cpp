...
constexpr uint32_t CURVE_POINT_NUM_MIN = 4;
constexpr uint32_t CURVE_POINT_NUM_MAX = 16;
...
int32_t HEVibratorDecoder::DecodeEffect(const RawFileDescriptor &rawFd, VibratePackage &pkg)
{
    ...
    switch (version) {
        ...
        default: {
            MISC_HILOGE("unsupported version %d", version);
            return ERROR;
        }
    }
    return SUCCESS;
...
int32_t HEVibratorDecoder::ParsePatternList(const JsonParser &parser, cJSON *patternListJSON, VibratePackage &pkg)
{
    ...
    if (size <= 0) {
        MISC_HILOGE("The size of pattern list is 0!");
        return ERROR;
    }
    ...
    for (int32_t i = 0; i < size; i++) {
        ...
        int32_t time = timeJSON->valueint;
        if (time <= previousPattternTime) {
            MISC_HILOGE("The value of absolute time %d is invalid!", time);
            return ERROR;
        }
...
int32_t HEVibratorDecoder::ParsePattern(const JsonParser &parser, cJSON *patternJSON, VibratePattern &pattern)
{
    ...
    for (int32_t i = 0; i < size; i++) {
        ...
        if (event.time < previousEventTime) {
            MISC_HILOGE("The value of absolute time %d is invalid!", event.time);
            return ERROR;
        }
...
int32_t HEVibratorDecoder::ParseEvent(const JsonParser &parser, cJSON *eventJSON, VibrateEvent &event)
{
    ...
    if (type == "transient") {
        ...
        event.duration = durationJSON->valueint;
    } else {
    ...
    if (indexJSON != NULL) {
        event.index = indexJSON->valueint;
    } else {
        event.index = 0;
    }
    cJSON *timeJSON = parser.GetObjectItem(eventJSON, "RelativeTime");
    CHKPR(timeJSON, ERROR);
    event.time = timeJSON->valueint;
...
int32_t HEVibratorDecoder::ParseCurve(const JsonParser &parser, cJSON *curveJSON, VibrateEvent &event)
{
    ...
    size_t size = parser.GetArraySize(curveJSON);
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        CHKPR(intensityJSON, ERROR);
        point.intensity = (intensityJSON->valuedouble) * CURVE_INTENSITY_SCALE;
        ...
        CHKPR(frequencyJSON, ERROR);
        point.frequency = frequencyJSON->valueint;
...
bool HEVibratorDecoder::CheckEventParameters(const VibrateEvent &event)
{
    ...
    if (event.tag == EVENT_TAG_TRANSIENT) {
        if (event.frequency < TRANSIENT_FREQUENCY_MIN || event.frequency > TRANSIENT_FREQUENCY_MAX) {
            MISC_HILOGE("The transient event frequency is out of range, frequency:%{public}d", event.frequency);
            return false;
        }
        ...
        if (event.frequency < CONTINUOUS_FREQUENCY_MIN || event.frequency > CONTINUOUS_FREQUENCY_MAX) {
            MISC_HILOGE("The continuous event frequency is out of range, frequency:%{public}d", event.frequency);
            return false;
        }
        if (event.duration <= 0 || event.duration > CONTINUOUS_DURATION_MAX) {
            MISC_HILOGE("The continuous event duration is out of range, duration:%{public}d", event.duration);
            return false;
        }
        ...
        if (event.points[0].time != 0 || event.points[0].intensity != 0) {
            MISC_HILOGE("The fist curve point is invalivd, time %{public}d, insentsity %{public}d",
                event.points[0].time, event.points[0].intensity);
            return false;
        }
